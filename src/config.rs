...
lazy_static::lazy_static! {
    ...
    //ID服务器，所有客户端生效
    pub static ref PROD_RENDEZVOUS_SERVER: RwLock<String> = RwLock::new((option_env!("RENDEZVOUS_SERVER").unwrap_or("rs-ny.rustdesk.com")).to_owned());
    pub static ref EXE_RENDEZVOUS_SERVER: RwLock<String> = RwLock::new((option_env!("RENDEZVOUS_SERVER").unwrap_or("rs-ny.rustdesk.com")).to_owned());
    ...
    pub static ref DEFAULT_SETTINGS: RwLock<HashMap<String, String>> = {
        ...
        map.insert("custom-rendezvous-server".to_string(), (option_env!("RENDEZVOUS_SERVER").unwrap_or("rs-ny.rustdesk.com")).to_string());
        //中继服务器
        map.insert("relay-server".to_string(), (option_env!("RELAY_SERVER").unwrap_or("rs-ny.rustdesk.com")).to_string());
        //API服务器
        map.insert("api-server".to_string(), (option_env!("API_SERVER").unwrap_or("https://admin.rustdesk.com")).to_string());
        //KEY "kmVguztfN7pwlsAoSF2AArSTLVbdSebUsPwGGmIvoyc="
        map.insert("key".to_string(), (option_env!("RS_PUB_KEY").unwrap_or("OeVuKk5nlHiXp+APNn0Y3pC1Iwpwn44JGqrQCsWqmBw=")).to_string());
        //PIN解锁，需要配合PIN修复代码块使用
        map.insert("unlock_pin".to_string(), (option_env!("DEFAULT_PASSWORD").unwrap_or("")).to_string());
    ...
    pub static ref HARD_SETTINGS: RwLock<HashMap<String, String>> = {
        let mut map = HashMap::new();
        //被控默认密码
        map.insert("password".to_string(), (option_env!("DEFAULT_PASSWORD").unwrap_or("")).to_string());
        RwLock::new(map)
    };
    pub static ref BUILTIN_SETTINGS: RwLock<HashMap<String, String>> = {
        let mut map = HashMap::new();
        //默认连接密码
        map.insert("default-connect-password".to_string(), (option_env!("DEFAULT_PASSWORD").unwrap_or("")).to_string());
        RwLock::new(map)
    };
}
